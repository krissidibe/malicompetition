  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex flex-1 h-full gap-6 ">
        ...
        >
          Etat du concours : {statutData[parseInt(result.competition.statut)].name}
        </span>
        <span className="text-[14px]  text-gray-500   ">
          Date : du {new Date(result.competition.startDateAt).toLocaleDateString("fr-FR")} au{" "}
          {new Date(result.competition.endDateAt).toLocaleDateString("fr-FR")}
        </span>
        <span className="text-[14px]  text-gray-500  ">
          L'age est comprise entre : {result.competition.ageMin} ans et {result.competition.ageMax} ans
        </span>
      
      </div>
      <p className="text-[14px] text-gray-500 mb-20">{parse(result.competition.content || "")}</p>
        </div>
